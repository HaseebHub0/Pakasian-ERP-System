...
class Role(BaseModel):
    role_name = models.CharField(max_length=100, unique=True, db_column='role_name')
...
class SystemUser(AbstractBaseUser, BaseModel):
    ...
    role_id = models.ForeignKey(Role, on_delete=models.PROTECT, db_column='role_id', null=True)
    ...
    # Override internal Django password field mapping to use custom password_hash field
    @property
    def password(self):
...
class ApiToken(BaseModel):
    user_id = models.ForeignKey(SystemUser, on_delete=models.CASCADE, db_column='user_id')
