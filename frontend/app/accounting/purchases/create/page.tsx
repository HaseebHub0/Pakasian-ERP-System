...
export default function CreatePurchase() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/accounting/purchases`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert('Purchase record created successfully!');
        router.push('/accounting/purchases');
      } else {
        const errorData = await response.json();
        alert(errorData.message || 'Error creating purchase record');
      }
    } catch (error) {
      console.error('Error creating purchase:', error);
      alert('Error creating purchase record');
    } finally {
      setLoading(false);
    }
  };
  ...
  const calculateTotal = () => {
    ...
    setFormData({...formData, total_cost: total.toFixed(2)});
  };
        ...
        <div className="bg-white border-2 border-black rounded-lg shadow-lg p-6">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="text-center md:text-left mb-4 md:mb-0">
              <h1 className="text-3xl font-bold text-black">Create Purchase Record</h1>
              <p className="text-gray-600 text-lg">Record raw material purchase and supplier invoice</p>
            </div>
            <Link
        ...
        <div className="bg-white border-2 border-black rounded-lg shadow-lg p-8">
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-lg text-black"
                  placeholder="e.g., Raw Potatoes, Flour, Oil"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-2 border-gray-400 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500 text-lg text-black bg-gray-100"
                  placeholder="0.00"
                  readOnly
                />
              </div>
            ...
            <div className="bg-gray-100 border-2 border-gray-400 rounded-lg p-4">
              <h3 className="text-lg font-bold text-black mb-3">Purchase Summary</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  <span className="font-bold text-black">Unit Cost:</span>
                  <p className="text-gray-600">${formData.unit_cost || '0.00'}</p>
                </div>
                <div>
                  <span className="font-bold text-black">Total Cost:</span>
                  <p className="text-red-600 font-bold">${formData.total_cost || '0.00'}</p>
                </div>
              </div>
            </div>
